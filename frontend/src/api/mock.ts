...
export const mockApi: Api = {
  ...
  async createKeyword(draft: KeywordDraft) {
    ...
    const created: Keyword = {
      ...
      searchWindowDays: draft.searchWindowDays,
      lectureCount: 0,
      lastRunAt: null,
  ...
  async getPipeline() {
    ...
    return {
      ...
      blog: {
        enabled: true,
        nextAt: iso(new Date(Date.now() + 8 * 60_000)),
        waiting: 12,
        posted: 7,
        // 상한에 안 닿은 평소 모습. 닿은 날은 "오늘 몫 다 씀"으로 바뀝니다.
        postedToday: 7,
        dailyCap: 30,
        // 세션은 살아 있는 모습으로. 죽은 모습은 "로그인 필요" 한 줄입니다.
        sessionBadSince: null,
        failed: 0,
        recent: [
          { at: iso(new Date(Date.now() - 6 * 60_000)), title: "DNA 계통수 분석과 이명법",
            category: "사이언스", postId: "197", url: "https://example.tistory.com/197" },
          { at: iso(new Date(Date.now() - 12 * 60_000)), title: "SGLang, vLLM, Llama.cpp의 아키텍처",
            category: "LLM 서빙 최적화", postId: "196", url: "https://example.tistory.com/196" },
          { at: iso(new Date(Date.now() - 15 * 60_000)), title: "미 연준 통화정책과 한국은행 금리·환율 대응",
            category: "경제", postId: "195", url: "https://example.tistory.com/195" },
          { at: iso(new Date(Date.now() - 20 * 60_000)), title: "3대 경제지표를 활용한 증권시장 분석",
            category: "경제", postId: "194", url: "https://example.tistory.com/194" },
          { at: iso(new Date(Date.now() - 29 * 60_000)), title: "AI가 경제와 노동시장에 미치는 영향",
            category: "AI", postId: "193", url: "https://example.tistory.com/193" },
        ],
      },
      // 스스로 손본 것 — 있을 때의 모습을 봐야 자리가 맞는지 압니다.
      upkeep: "08-21 04:12 · yt-dlp 2026.7.4 → 2026.8.19 올렸습니다",
