...
# 일시적이었다는 표시. 사유 문장에 이게 들어 있으면 영상 탓이 아닙니다 —
# 멤버십 전용·비공개·삭제 같은 영구 사유와 갈리는 자리입니다.
TEMPORARY = ("403", "forbidden", "429", "too many requests", "timed out", "timeout", "연결")
...
def main() -> int:
    ...
    try:
        ...
        if no_captions:
            ...
            label = "일시적"
            targets = [
                v for v in rows if any(m in (v.state_reason or "").lower() for m in TEMPORARY)
            ]
