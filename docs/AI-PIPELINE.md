...
**두 경로가 있고, 비용 모델이 완전히 달라집니다.**

| 경로 | 설정 | 과금 | 컨테이너 적합성 |
|---|---|---|---|
| **API 키** | `ANTHROPIC_API_KEY` | 토큰 단위 종량제 (§5.3 표) | ✅ 문서화된 비대화형 표준 경로 |
| **구독 (OAuth)** | 로그인 자격증명 필요 | 구독료 정액 + 사용량 한도 | △ 컨테이너에 자격증명 반입·갱신 문제 |

> **"헤드리스로 돌리면 API 비용이 안 나간다"는 전제라면 여기서 검증이 필요합니다.** 비대화형 실행의 문서화된 인증 방법은 `ANTHROPIC_API_KEY`이고, 이 경우 과금은 **API 직접 호출과 동일한 토큰 단가**입니다. 구독 자격증명으로 돌리는 것이 가능한지, 컨테이너에서 토큰 갱신이 되는지, 그리고 그것이 이용약관상 허용되는 사용 형태인지를 **M0에서 실제로 확인한 뒤** 비용 모델을 확정하세요. 이 결과에 따라 §5.3 표가 유효할 수도, 무의미해질 수도 있습니다.

구독 경로로 확정될 경우 §5의 비용 가드는 **레이트리밋 가드**로 성격이 바뀝니다.
