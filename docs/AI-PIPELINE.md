...
**상한도 회사별로 겁니다.** 대시보드의 `5시간 토큰` 패널에 회사마다 한
줄씩 나오고, 숫자를 눌러 그 회사만 바꿉니다(단위는 백만). 자기 값이 없는
회사는 공용 값을 물려받고 `공용` 이라고 표시됩니다 — 값이 왜 그런지 안
보이면 "분명 바꿨는데 왜 안 변하지"가 됩니다. `공용으로` 를 누르면 자기
값을 지우고 다시 물려받습니다.
...
상단 미터의 합계 상한은 **회사별 상한의 합**입니다. 하나라도 무제한이면
합계도 무제한입니다 — 남은 것들만 더해 놓으면 미터가 실제보다 빨리 차
보여서, 아직 여유가 있는데도 아껴 쓰게 만듭니다.
...
파이프를 물려받습니다. 맏이만 `kill()` 하면 도우미가 쓰기 끝을 쥔 채 남아
EOF 가 오지 않습니다. 게다가 **타임아웃 뒤에 `communicate()` 를 다시 부르면
안 됩니다** — 취소된 첫 호출이 파이프를 물고 있어서 두 번째 호출이 영영
끝나지 않습니다.

이 둘이 겹쳐서 워커가 **72분을 멎어** 있었습니다. 자식은 이미 죽었는데
죽은 자식의 파이프 두 개(`lsof` 로 확인)를 붙든 채였고, 그동안 요약은
0건이었습니다. 락은 그 워커가 쥐고 있어서 좀비 회수도 못 돌았고, 붙들고
있던 영상은 다른 회사의 6시간 유예에 걸려 그대로 갇혀 있었습니다.

  start_new_session=True   자기 프로세스 그룹으로 띄웁니다
  os.killpg(...)           그룹째 SIGTERM → SIGKILL
  wait_for(proc.wait(), 5) 정리하는 동안에도 상한을 둡니다
